...
const ParticleBackground = () => {
  ...
  useEffect(() => {
    ...
    const nodeCount = Math.min(120, Math.floor((canvas.width * canvas.height) / 9500));
    ...
    const animate = () => {
      ...
      ctx.fillStyle = "rgba(2, 9, 20, 0.38)";
      ...
      orbs.forEach(orb => {
        ...
        const pulseAlpha = 0.02 + Math.sin(orb.phase) * 0.012;
        const gradient = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, orb.radius);
        gradient.addColorStop(0, `rgba(${orb.color}, ${pulseAlpha * 1.5})`);
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < 150) {
            const lineAlpha = 0.14 * (1 - dist / 150);
            ...
            ctx.lineWidth = 0.45 + connectionPulse * 0.25;
            ctx.stroke();

            // Draw data packet traveling along connection occasionally
            if (Math.sin(time * 2.4 + i + j) > 0.98) {
              const t = (Math.sin(time * 5 + i) + 1) / 2;
              const px = p.x + dx * t;
              const py = p.y + dy * t;
              ctx.beginPath();
              ctx.arc(px, py, 1.5, 0, Math.PI * 2);
              ctx.fillStyle = "rgba(50, 255, 150, 0.55)";
              ctx.fill();
            }
          }
        }
      ...
      if (mouse.x > 0 && mouse.y > 0) {
        ...
        cursorGlow.addColorStop(0, "rgba(0, 150, 255, 0.05)");
        cursorGlow.addColorStop(0.5, "rgba(0, 150, 255, 0.02)");
